...
int main(int argc, char* argv[]) {
  ...
  OpenDDS::Security::SecurityConfig_rch config = TheSecurityRegistry->default_config();
  const bool ok = plugin && config && config->name() == "pqsec" && config->get_authentication() &&
                  config->get_access_control() && config->get_crypto_key_factory() &&
                  config->get_crypto_key_exchange() && config->get_crypto_transform() &&
                  config->get_utility();
