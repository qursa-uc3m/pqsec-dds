...
struct Participant {
  ...
  Participant(unsigned char suffix, const char* name)
    : candidate(OpenDDS::DCPS::GUID_UNKNOWN)
    , adjusted(OpenDDS::DCPS::GUID_UNKNOWN)
    , local(DDS::HANDLE_NIL)
    , remote(DDS::HANDLE_NIL)
    , handshake(DDS::HANDLE_NIL)
  {
...
bool check(bool condition, const char* message, const DDS::Security::SecurityException& ex)
{
  ...
  std::cerr << message;
  if (ex.message.in() && ex.message.in()[0]) std::cerr << ": " << ex.message.in();
  std::cerr << '\n';
  return false;
}
...
bool initialize(Participant& participant)
{
  if (!check(participant.authentication.validate_local_identity(
      participant.local, participant.adjusted, 0, participant.qos,
      participant.candidate, participant.exception) == VALIDATION_OK,
      "validate_local_identity failed", participant.exception)) return false;
  if (!check(participant.authentication.get_identity_token(
      participant.identity_token, participant.local, participant.exception),
      "get_identity_token failed", participant.exception)) return false;
  ...
  if (!check(participant.authentication.set_permissions_credential_and_token(
      ...
      "set_permissions_credential_and_token failed", participant.exception)) return false;
...
int main()
{
  ...
  if (!initialize(a) || !initialize(b)) return 1;
  ...
  if (!check((a_role == VALIDATION_PENDING_HANDSHAKE_REQUEST &&
             ...
             "participants did not choose complementary handshake roles", a.exception)) return 1;

  DDS::Security::AuthRequestMessageToken ignored;
  if (a.authentication.validate_remote_identity(
        a.remote, ignored, b.auth_request, a.local, b.identity_token, b.adjusted,
        a.exception) == DDS::Security::VALIDATION_FAILED ||
      b.authentication.validate_remote_identity(
        b.remote, ignored, a.auth_request, b.local, a.identity_token, a.adjusted,
  ...
  if (!check(initiator->authentication.begin_handshake_request(
      ...
      initiator->participant_data, initiator->exception) == VALIDATION_PENDING_HANDSHAKE_MESSAGE,
      "begin_handshake_request failed", initiator->exception)) return 1;
  ...
  if (!check(replier->authentication.begin_handshake_reply(
      ...
      replier->participant_data, replier->exception) == VALIDATION_PENDING_HANDSHAKE_MESSAGE,
      "begin_handshake_reply failed", replier->exception)) return 1;

  DDS::Security::HandshakeMessageToken final_message;
  if (!check(initiator->authentication.process_handshake(
      final_message, reply, initiator->handshake,
      initiator->exception) == VALIDATION_OK_FINAL_MESSAGE,
      "reply processing failed", initiator->exception)) return 1;
  DDS::Security::HandshakeMessageToken unused;
  if (!check(replier->authentication.process_handshake(
      unused, final_message, replier->handshake,
      replier->exception) == VALIDATION_OK,
      "final processing failed", replier->exception)) return 1;

  DDS::Security::SharedSecretHandle_var first = initiator->authentication.get_shared_secret(
    initiator->handshake, initiator->exception);
  DDS::Security::SharedSecretHandle_var second = replier->authentication.get_shared_secret(
    replier->handshake, replier->exception);
  if (!check(first.in() && second.in(), "shared secret unavailable", initiator->exception)) return 1;
  ...
  if (!check(first_bytes->length() != 0 &&
             ...
             "KEM shared secrets differ", initiator->exception)) return 1;
  return 0;
}
