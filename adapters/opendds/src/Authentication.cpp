...
{
  if (!value.length() || value[value.length() - 1] != 0) return std::string();
                       ...
                       const OpenDDS::DCPS::GUID_t& guid)
{
  ...
  if (certificate.subject_name_digest(digest) || digest.size() < 6) return false;
  const unsigned char* bytes = reinterpret_cast<const unsigned char*>(&guid);
  if ((bytes[0] & 0x80) != 0x80 ||
      (bytes[0] & 0x7f) != OpenDDS::Security::SSL::offset_1bit(&digest[0], 0)) return false;
  for (size_t i = 1; i != 6; ++i) {
    if (bytes[i] != OpenDDS::Security::SSL::offset_1bit(&digest[0], i)) return false;
  }
  return true;
}

bool participant_data_has_guid(const DDS::OctetSeq& data,
                               const OpenDDS::DCPS::GUID_t& expected)
{
  ...
  if (!(serializer >> parameters)) return false;
...
public:
  ...
  ~SharedSecret()
  {
    if (secret_.length()) OPENSSL_cleanse(secret_.get_buffer(), secret_.length());
  }

  DDS::OctetSeq* challenge1() { return new DDS::OctetSeq(challenge1_); }
  DDS::OctetSeq* challenge2() { return new DDS::OctetSeq(challenge2_); }
  DDS::OctetSeq* sharedSecret() { return new DDS::OctetSeq(secret_); }
...
struct Authentication::Handshake {
  ...
  ~Handshake()
  {
    if (secret.length()) OPENSSL_cleanse(secret.get_buffer(), secret.length());
  }
  bool initiator;
  ...
  const OpenDDS::DCPS::GUID_t& candidate, DDS::Security::SecurityException& ex)
{
  ...
  if (!kem_algorithm(selected_kem) || !openssl_supports(selected_kem)) {
    fail(ex, "PQSec requested an unavailable KEM algorithm");
    return DDS::Security::VALIDATION_FAILED;
  }
...
CORBA::Boolean Authentication::get_identity_token(DDS::Security::IdentityToken& token,
  DDS::Security::IdentityHandle handle, DDS::Security::SecurityException& ex)
{
  std::lock_guard<std::mutex> guard(mutex_);
  LocalPtr identity = local(handle);
  if (!identity) { fail(ex, "Unknown local identity"); return false; }
  ...
  writer.add_property("dds.cert.algo", identity->credentials->get_participant_cert().keypair_algo());
...
{
  std::lock_guard<std::mutex> guard(mutex_);
  if (!local(handle)) { fail(ex, "Unknown local identity"); return false; }
  return true;
}
...
DDS::Security::ValidationResult_t Authentication::validate_remote_identity(
  ...
  DDS::Security::IdentityHandle local_handle,
  const DDS::Security::IdentityToken& identity_token,
  const OpenDDS::DCPS::GUID_t& remote_guid, DDS::Security::SecurityException& ex)
{
  ...
  RemotePtr identity;
  if (remote_handle != DDS::HANDLE_NIL) identity = remote(remote_handle);
  ...
  const DDS::OctetSeq& participant_data, DDS::Security::SecurityException& ex)
{
  ...
  if (!hs->kem.generate(error) || !hs->kem.public_key(key, error)) {
    fail(ex, error); return DDS::Security::VALIDATION_FAILED;
  }
  ...
  hs->request = message;
  if (handle == DDS::HANDLE_NIL) handle = next_handle();
...
bool Authentication::validate_remote_credentials(Handshake& hs, const DDS::Security::Token& token,
  DDS::OctetSeq& hash, DDS::Security::SecurityException& ex)
{
  ...
  const std::string signature_algorithm = string_value(reader.get_bin_property_value("c.dsign_algo"));
  ...
  if (!mldsa_algorithm(signature_algorithm) || selected_kem != hs.local->kem_algorithm) {
    fail(ex, "Unsupported or mismatched PQSec algorithms"); return false;
  }
  ...
  if (credential_hash(certificate_data, permissions, participant_data,
                      signature_algorithm, selected_kem, hash)) {
    fail(ex, "Failed to hash remote PQSec credentials"); return false;
  }
  ...
  const DDS::OctetSeq& participant_data, DDS::Security::SecurityException& ex)
{
  ...
  if (!local_identity || !remote_identity || !class_is(request, REQUEST_CLASS_ID)) {
    fail(ex, "Invalid PQSec handshake request"); return DDS::Security::VALIDATION_FAILED;
  }
  ...
  DDS::BinaryPropertySeq transcript = reply_transcript(
    hs->hash2, hs->challenge2, hs->ciphertext, hs->challenge1, hs->public_key, hs->hash1);
  ...
  if (OpenDDS::Security::SSL::sign_serialized(
    ...
    fail(ex, "ML-DSA reply signature failed"); return DDS::Security::VALIDATION_FAILED;
  }
  ...
  DDS::Security::HandshakeHandle handle, DDS::Security::SecurityException& ex)
{
  ...
  if (pos == handshakes_.end()) {
    fail(ex, "Unknown PQSec handshake handle"); return DDS::Security::VALIDATION_FAILED;
  }
  ...
  if (hs.initiator && class_is(input, REPLY_CLASS_ID)) {
    ...
    if (!equal(reader.get_bin_property_value("challenge1"), hs.challenge1) ||
      ...
      fail(ex, "PQSec reply challenge mismatch"); return DDS::Security::VALIDATION_FAILED;
    }
    DDS::BinaryPropertySeq reply_data = reply_transcript(
      hs.hash2, hs.challenge2, hs.ciphertext, hs.challenge1, hs.public_key, hs.hash1);
    if (OpenDDS::Security::SSL::verify_serialized(
          reply_data, *hs.remote_certificate, reader.get_bin_property_value("signature"))) {
    ...
    hs.secret = octets(secret);
    if (!secret.empty()) OPENSSL_cleanse(&secret[0], secret.size());
    DDS::BinaryPropertySeq final_data = final_transcript(
      hs.hash1, hs.challenge1, hs.public_key, hs.challenge2, hs.ciphertext, hs.hash2);
    DDS::OctetSeq signature;
    if (OpenDDS::Security::SSL::sign_serialized(
          final_data, hs.local->credentials->get_participant_private_key(), signature)) {
      fail(ex, "ML-DSA final signature failed"); return DDS::Security::VALIDATION_FAILED;
    }
  ...
  if (!hs.initiator && class_is(input, FINAL_CLASS_ID)) {
    ...
    DDS::BinaryPropertySeq final_data = final_transcript(
      hs.hash1, hs.challenge1, hs.public_key, hs.challenge2, hs.ciphertext, hs.hash2);
    if (!hs.remote_certificate || OpenDDS::Security::SSL::verify_serialized(
          final_data, *hs.remote_certificate, reader.get_bin_property_value("signature"))) {
...
DDS::Security::SharedSecretHandle* Authentication::get_shared_secret(
  DDS::Security::HandshakeHandle handle, DDS::Security::SecurityException& ex)
{
  std::lock_guard<std::mutex> guard(mutex_);
  std::map<DDS::Security::HandshakeHandle, HandshakePtr>::iterator pos = handshakes_.find(handle);
  if (pos == handshakes_.end() ||
    ...
    fail(ex, "PQSec shared secret requested before authentication completed"); return 0;
  }
  ...
  DDS::Security::SecurityException& ex)
{
  ...
  if (pos == handshakes_.end() || !pos->second->remote_certificate ||
      (pos->second->state != DDS::Security::VALIDATION_OK &&
       pos->second->state != DDS::Security::VALIDATION_OK_FINAL_MESSAGE)) {
    fail(ex, "PQSec peer credentials are unavailable"); return false;
  }
  OpenDDS::Security::TokenWriter writer(token, BASE_CLASS_ID);
...
CORBA::Boolean Authentication::set_listener(DDS::Security::AuthenticationListener_ptr listener,
  DDS::Security::SecurityException& ex)
{
  if (!listener) { fail(ex, "Null authentication listener"); return false; }
...
CORBA::Boolean Authentication::return_identity_token(const DDS::Security::IdentityToken&,
  DDS::Security::SecurityException&) { return true; }
CORBA::Boolean Authentication::return_identity_status_token(const DDS::Security::IdentityStatusToken&,
  DDS::Security::SecurityException&) { return true; }
CORBA::Boolean Authentication::return_authenticated_peer_credential_token(
  const DDS::Security::AuthenticatedPeerCredentialToken&, DDS::Security::SecurityException&) { return true; }

CORBA::Boolean Authentication::return_handshake_handle(DDS::Security::HandshakeHandle handle,
  DDS::Security::SecurityException& ex)
{
  std::lock_guard<std::mutex> guard(mutex_);
  if (!handshakes_.erase(handle)) { fail(ex, "Unknown PQSec handshake handle"); return false; }
  return true;
}
...
{
  std::lock_guard<std::mutex> guard(mutex_);
  if (locals_.erase(handle) || remotes_.erase(handle)) return true;
  fail(ex, "Unknown PQSec identity handle"); return false;
}
