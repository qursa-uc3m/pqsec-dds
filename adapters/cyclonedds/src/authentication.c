 ...
 * for DDS security through OpenSSL providers. Based on CycloneDDS built-in
 * authentication plugin and DDS Security specification v1.1.
 */
...
#ifdef PQ_CRYPTO
  OSSL_PROVIDER *fallback_provider;
#endif
...
static bool ensure_selected_kem_available(dds_security_authentication_impl *auth) {
  const AuthenticationAlgoKind_t kind = get_default_kem_algorithm_kind();
  const char *provider_name = getenv("PQSEC_OPENSSL_PROVIDER");

  if (kind == AUTH_ALGO_KIND_UNKNOWN) {
    DBG_ERR("Unknown configured KEM algorithm: %s\n", get_default_kem_name());
    return false;
  }
  if (pqsec_kem_is_available(kind)) {
    return true;
  }

  if (!provider_name || provider_name[0] == '\0') {
    provider_name = PQSEC_FALLBACK_PROVIDER;
  }
  if (!provider_name || provider_name[0] == '\0') {
    DBG_ERR("OpenSSL does not expose KEM %s and no fallback provider is configured\n",
            get_default_kem_openssl_name());
    return false;
  }

  auth->fallback_provider = OSSL_PROVIDER_try_load(NULL, provider_name, 1);
  if (!auth->fallback_provider) {
    DBG_ERR("Failed to load OpenSSL provider '%s' for KEM %s\n", provider_name,
            get_default_kem_openssl_name());
    return false;
  }
  if (!pqsec_kem_is_available(kind)) {
    DBG_ERR("OpenSSL provider '%s' does not expose KEM %s\n", provider_name,
            get_default_kem_openssl_name());
    OSSL_PROVIDER_unload(auth->fallback_provider);
    auth->fallback_provider = NULL;
    return false;
  }

  DBG_INFO("Loaded OpenSSL provider '%s' for KEM %s\n", provider_name,
           get_default_kem_openssl_name());
  return true;
}
#endif
...
#ifdef PQ_CRYPTO
    pqsec_kem_keypair_free(handshake->kem_keypair);
    if (handshake->kem_ct) {
      pqsec_kem_buffer_free(handshake->kem_ct);
      handshake->kem_ct = NULL;
    }
    if (handshake->kem_ss) {
      pqsec_kem_secret_free(handshake->kem_ss, handshake->kem_ss_len);
...
DDS_Security_ValidationResult_t validate_local_identity(
  ...
  if (implementation->trustedCAList.length > 0) {
    ...
    if (digest_result) {
      for (unsigned i = 0; i < implementation->trustedCAList.length; ++i) {
        digest_result = X509_digest(implementation->trustedCAList.buffer[i], digest,
                                    hash_buffer_trusted, &size);
        if (digest_result && memcmp(hash_buffer_trusted, hash_buffer, 20) == 0) {
    ...
    if (result != DDS_SECURITY_VALIDATION_OK) {
      ...
      DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_CA_NOT_TRUSTED_CODE,
                                 DDS_SECURITY_VALIDATION_FAILED,
                                 DDS_SECURITY_ERR_CA_NOT_TRUSTED_MESSAGE);
      goto err_identity_ca_not_trusted;
    }
  ...
  if (X509_check_private_key(identityCert, privateKey) != 1) {
    DBG_ERR("Private key does not match the identity certificate\n");
    DDS_Security_Exception_set_with_openssl_error(
        ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE,
        DDS_SECURITY_VALIDATION_FAILED, "Private key does not match identity certificate: ");
    goto err_inv_crl;
  }
  ...
  if (get_adjusted_participant_guid(identityCert, candidate_participant_guid,
                                    adjusted_participant_guid, ex) != DDS_SECURITY_VALIDATION_OK) {
...
DDS_Security_boolean get_identity_token(dds_security_authentication *instance,
  ...
  identity_token->class_id = ddsrt_strdup(DDS_AUTHTOKEN_CLASS_ID);
...
validate_remote_identity_token(const LocalIdentityInfo *localIdent,
  ...
  if (strcmp(DDS_AUTHTOKEN_CLASS_ID, token->class_id) != 0) {
  ...
  DBG_TRACE("validate_remote_identity_token succeeded\n");
  return DDS_SECURITY_VALIDATION_OK;
}
...
validate_auth_request_token(const DDS_Security_IdentityToken *token,
  ...
  if (strcmp(token->class_id, DDS_AUTHTOKEN_AUTHREQ_CLASS_ID) != 0) {
  ...
  for (index = 0; index < token->binary_properties._length; index++) {
    ...
    if (token->binary_properties._buffer[index].name &&
        strncmp(token->binary_properties._buffer[index].name, DDS_AUTHTOKEN_PROP_FUTURE_CHALLENGE,
                len) == 0) {
...
static void fill_auth_request_token(DDS_Security_AuthRequestMessageToken *token,
  ...
  token->class_id = ddsrt_strdup(DDS_AUTHTOKEN_AUTHREQ_CLASS_ID);
  DBG_TRACE("Set token class_id to '%s'\n", DDS_AUTHTOKEN_AUTHREQ_CLASS_ID);
...
DDS_Security_ValidationResult_t validate_remote_identity(
  ...
  if (!instance || !remote_identity_handle || !local_auth_request_token || !remote_identity_token ||
  ...
  *remote_identity_handle = IDENTITY_HANDLE(remoteIdent);
  ...
  DDS_Security_ValidationResult_t result = guid_comparison < 0
...
begin_handshake_request(dds_security_authentication *instance,
  ...
  if (!pqsec_kem_keypair_generate(localIdent->kagreeAlgoKind, &handshake->kem_keypair)) {
    DBG_ERR("Failed to generate PQ KEM keys\n");
    DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE,
                               DDS_SECURITY_VALIDATION_FAILED,
                               "OpenSSL EVP failed to generate %s keypair",
                               get_authentication_algo(localIdent->kagreeAlgoKind));
    goto err_gen_dh_keys;
  }

  dhPubKeyData = handshake->kem_keypair->public_key;
  dhPubKeyDataSize = (uint32_t)handshake->kem_keypair->public_key_size;
  ...
  handshake_message->class_id = ddsrt_strdup(DDS_AUTHTOKEN_REQ_CLASS_ID);
...
static X509 *load_X509_certificate_from_binprop(const DDS_Security_BinaryProperty_t *prop,
  ...
  if (load_X509_certificate_from_data((char *)prop->value._buffer, (int)prop->value._length, &cert,
                                      oqs_libctx, ex) != DDS_SECURITY_VALIDATION_OK) {
  ...
  if (trusted_ca_list->length == 0) {
    ...
    for (unsigned i = 0; i < trusted_ca_list->length; ++i) {
      ...
      if ((result = verify_certificate(cert, trusted_ca_list->buffer[i], NULL, oqs_libctx, ex)) ==
          DDS_SECURITY_VALIDATION_OK) {
...
static DDS_Security_BinaryProperty_t *create_dhkey_property(const char *name, EVP_PKEY *pkey,
  ...
  uint32_t len = 0;
  DDS_Security_ValidationResult_t rv = dh_public_key_to_oct(pkey, kagreeAlgoKind, &data, &len, ex);
...
static DDS_Security_ValidationResult_t validate_handshake_token_impl(
  ...
  DDSRT_UNUSED_ARG(dh1_ref);
  DDSRT_UNUSED_ARG(dh2_ref);
#else
  OSSL_LIB_CTX *oqs_ctx = NULL;
  DDSRT_UNUSED_ARG(auth_impl);
#endif
  ...
  switch (token_type) {
  case HS_TOKEN_REQ:
    token_class_id = DDS_AUTHTOKEN_REQ_CLASS_ID;
    DBG_TRACE("Token type: REQUEST\n");
    break;
  case HS_TOKEN_REPLY:
    token_class_id = DDS_AUTHTOKEN_REPLY_CLASS_ID;
    DBG_TRACE("Token type: REPLY\n");
    break;
  case HS_TOKEN_FINAL:
    token_class_id = DDS_AUTHTOKEN_FINAL_CLASS_ID;
  ...
  /* Validate class ID */
  if (!token->class_id || strcmp(token_class_id, token->class_id) != 0) {
  ...
  if (token_type == HS_TOKEN_REQ || token_type == HS_TOKEN_REPLY) {
    ...
    if ((c_dsign_algo =
             find_required_nonempty_binprop(token, DDS_AUTHTOKEN_PROP_C_DSIGN_ALGO, ex)) == NULL) {
    ...
    if ((dsignAlgoKind = get_dsign_algo_from_octseq(&c_dsign_algo->value)) ==
      ...
      return set_exception(ex, "process_handshake: HandshakeMessageToken property %s not supported",
                           DDS_AUTHTOKEN_PROP_C_DSIGN_ALGO);
    }
    DBG_TRACE("Determined dsignAlgoKind = %d\n", dsignAlgoKind);

    if (dsignAlgoKind != get_authentication_algo_kind(identityCert)) {
      DBG_ERR("C_DSIGN_ALGO does not match the certificate public key\n");
      return set_exception(ex,
                           "process_handshake: HandshakeMessageToken property %s does not "
    ...
    if (kagreeAlgoKind != relation->localIdentity->kagreeAlgoKind) {
      DBG_ERR("C_KAGREE_ALGO does not match the locally configured KEM\n");
      return set_exception(ex,
                           "process_handshake: HandshakeMessageToken property %s does not "
                           "match the locally configured KEM",
                           DDS_AUTHTOKEN_PROP_C_KAGREE_ALGO);
    }

    /* Calculate hash value */
    {
      const DDS_Security_BinaryProperty_t *binary_properties[] = {c_id, c_perm, c_pdata,
                                                                  c_dsign_algo, c_kagree_algo};
  ...
  if (token_type == HS_TOKEN_REQ) {
    ...
    if ((dh1 = find_required_nonempty_binprop(token, DDS_AUTHTOKEN_PROP_KEM_PUBLIC, ex)) == NULL) {
    ...
    if (validate_kem_public_key_in_token(dh1, kagreeAlgoKind, ex) != DDS_SECURITY_VALIDATION_OK) {
    ...
    if (dh_oct_to_public_key(&pdhkey_req, kagreeAlgoKind, dh1->value._buffer, dh1->value._length,
    ...
    if (validate_kem_public_key_in_token(dh1, validation_algo, ex) != DDS_SECURITY_VALIDATION_OK) {
  ...
  if ((challenge1 = find_required_binprop_exactsize(token, DDS_AUTHTOKEN_PROP_CHALLENGE1,
                                                    sizeof(AuthenticationChallenge), ex)) == NULL) {
  ...
  if (token_type == HS_TOKEN_REPLY || token_type == HS_TOKEN_FINAL) {
    ...
    if ((kem_ciphertext = find_required_nonempty_binprop(token, DDS_AUTHTOKEN_PROP_KEM_CIPHERTEXT,
                                                         ex)) == NULL) {
  ...
  /* Validate challenge consistency with stored challenges */
  {
    ...
    if (relation->rchallenge) {
      if (memcmp(relation->rchallenge->value, rc->value._buffer, sizeof(AuthenticationChallenge)) !=
          0) {
  ...
  if (token_type == HS_TOKEN_REPLY || token_type == HS_TOKEN_FINAL) {
    ...
    if ((hash_c2 =
      ...
      if (hash_c2->value._length != sizeof(HashValue_t) ||
        ...
        return set_exception(ex,
                             "process_handshake: HandshakeMessageToken property hash_c2 invalid");
      }
    ...
    if (memcmp(relation->lchallenge->value, lc->value._buffer, sizeof(AuthenticationChallenge)) !=
        0) {
  ...
  if (token_type == HS_TOKEN_REPLY || token_type == HS_TOKEN_FINAL) {
    ...
    if (!EVP_PKEY_can_sign(public_key)) {
      ...
      return set_exception(
          ex, "Certificate contains non-signing key - need ML-DSA certificate for signatures");
    }
    ...
    if (token_type == HS_TOKEN_REPLY) {
      ...
      DDS_Security_ValidationResult_t result =
          validate_pq_signature(public_key, binary_properties, impl->include_optional ? 6 : 4,
    ...
    DDS_Security_ValidationResult_t result;
    if (token_type == HS_TOKEN_REPLY) {
      result = validate_signature(public_key,
                                  (const DDS_Security_BinaryProperty_t *[]){
...
begin_handshake_reply(dds_security_authentication *instance,
  ...
  if (!handshake->kem_ct || !handshake->kem_ss) {
    ...
    if (!pqsec_kem_encapsulate(
            remoteIdent->kagreeAlgoKind, binary_property_kem_public->value._buffer,
            binary_property_kem_public->value._length, &handshake->kem_ct, &handshake->kem_ct_len,
            &handshake->kem_ss, &handshake->kem_ss_len)) {
      DBG_ERR("KEM encapsulation failed in begin_handshake_reply\n");
      DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE,
                                 DDS_SECURITY_VALIDATION_FAILED,
                                 "OpenSSL EVP KEM encapsulation failed");
      goto err_encap_kem_key;
    }
  ...
  /* Calculate the PQ signature using ML-DSA */
  {
    ...
    if (impl->include_optional) {
      ...
      if (create_pq_signature(signing_key, binary_properties,
                              PLUGIN_HANDSHAKE_SIGNATURE_CONTENT_SIZE, &kem_signature,
                              &kem_signature_len, ex, &sign_public_key, localIdent->dsignAlgoKind,
    ...
    DBG_CRYPTO("Signature property set\n");
  }
#else
  /* Calculate the classical signature */
  {
    ...
    DDS_Security_BinaryProperty_set_by_ref(&tokens[tokidx++], DDS_AUTHTOKEN_PROP_SIGNATURE, sign,
                                           (uint32_t)signlen);
  ...
  (void)ddsrt_hh_add(impl->objectHash, handshake);
  handshake_message_out->class_id = ddsrt_strdup(DDS_AUTHTOKEN_REPLY_CLASS_ID);
...
static bool generate_shared_secret(const HandshakeInfo *handshake, unsigned char **shared_secret,
  ...
  *shared_secret = NULL;
  ...
  if (EVP_PKEY_derive(ctx, secret, &skeylen) <= 0) {
    ...
    DDS_Security_Exception_set_with_openssl_error(
        ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE,
        DDS_SECURITY_VALIDATION_FAILED, "process_handshake: Could not compute the shared secret: ");
    goto fail_derive;
  }
...
process_handshake(dds_security_authentication *instance,
  ...
  const uint32_t tsz = impl->include_optional ? 7 : 5;
  ...
  case CREATEDREQUEST:
    ...
    /* PQ: Handle REPLY token processing for REQUEST creator */
    {
      ...
      kem_public = DDS_Security_DataHolder_find_binary_property(handshake_message_in,
                                                                DDS_AUTHTOKEN_PROP_KEM_PUBLIC);
      if (!kem_public) {
        goto err_inv_token;
      }
      ...
      if (!pqsec_kem_decapsulate(handshake->kem_keypair, kem_ciphertext->value._buffer,
                                 kem_ciphertext->value._length, &handshake->kem_ss,
                                 &handshake->kem_ss_len)) {
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE,
                                   DDS_SECURITY_VALIDATION_FAILED,
                                   "OpenSSL EVP KEM decapsulation failed");
        goto err_inv_token;
      }
    ...
    /* Create FINAL token */
    {
      ...
      if (impl->include_optional) {
        ...
        DDS_Security_BinaryProperty_set_by_value(&tokens[idx++], DDS_AUTHTOKEN_PROP_DH1,
                                                 dh1_gen->value._buffer, dh1_gen->value._length);
      ...
      DDS_Security_BinaryProperty_set_by_value(&tokens[idx++], DDS_AUTHTOKEN_PROP_KEM_CIPHERTEXT,
                                               kem_ciphertext->value._buffer,
                                               kem_ciphertext->value._length);
      ...
      /* Calculate the signature */
      {
        ...
        if (impl->include_optional) {
          const DDS_Security_BinaryProperty_t
              *binary_properties[PLUGIN_HANDSHAKE_SIGNATURE_CONTENT_SIZE_OPT] = {
                  hash_c1_val, challenge1, kem_public, challenge2, kem_ciphertext, hash_c2_val};
          result = create_pq_signature(signing_key, binary_properties,
                                       PLUGIN_HANDSHAKE_SIGNATURE_CONTENT_SIZE_OPT, &kem_signature,
                                       &kem_signature_len, ex, NULL,
        ...
        DDS_Security_BinaryProperty_set_by_ref(&tokens[idx++], DDS_AUTHTOKEN_PROP_SIGNATURE, sign,
                                               (uint32_t)signlen);
#endif
      }

      handshake_message_out->class_id = ddsrt_strdup(DDS_AUTHTOKEN_FINAL_CLASS_ID);
  ...
  case CREATEDREPLY:
    ...
    if (validate_handshake_token(handshake_message_in, HS_TOKEN_FINAL, handshake, NULL, NULL, NULL,
                                 impl, ex) != DDS_SECURITY_VALIDATION_OK) {
...
DDS_Security_boolean get_authenticated_peer_credential_token(
  ...
  if (!instance || !handshake_handle || !peer_credential_token) {
    ...
    DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_INVALID_PARAMETER_CODE,
                               0, DDS_SECURITY_ERR_INVALID_PARAMETER_MESSAGE);
    return false;
  }
  ...
  peer_credential_token->class_id = ddsrt_strdup(DDS_AUTHTOKEN_CLASS_ID);
...
static void invalidate_remote_related_objects(dds_security_authentication_impl *impl,
  ...
  for (IdentityRelation *relation = ddsrt_hh_iter_first(remoteIdentity->linkHash, &it);
    ...
    HandshakeInfo *handshake = find_handshake(impl, SECURITY_OBJECT_HANDLE(relation->localIdentity),
...
#ifdef PQ_CRYPTO
  if (!ensure_selected_kem_available(authentication)) {
...
#ifdef PQ_CRYPTO
    if (authentication->fallback_provider) {
      OSSL_PROVIDER_unload(authentication->fallback_provider);
      authentication->fallback_provider = NULL;
    }
#endif
